...
public sealed class OnnxDynamicDetector : IDynamicVisionProvider
{
    ...
    public OnnxDynamicDetector(IOnnxInferenceEngine engine, double confidenceThreshold, long observationTtlMs, SelfIdentityTracker? identityTracker = null)
    {
    ...
    public async ValueTask<DynamicVisionResult> ObserveDynamicAsync(CapturedFrame frame, CancellationToken cancellationToken)
    {
        ...
        SelfIdentityResult identity = identityTracker.Update(valid, frame.Metadata.CapturedAtMonoMs, monsters.Count > 0);
