...
显示阈值与动作阈值必须分离：新鲜且达到低显示阈值的检测可以绘制，不能因为尚未通过动作门禁而让用户看不到识别结果。动作侧仍须通过唯一性、多帧位置一致性、框尺度和受控位移反馈形成融合身份置信度；单帧原始分数不能单独放行动作。
...
原生预览必须以 React 上报的中央孔径为唯一布局目标，正确处理 CSS 像素、浏览器客户区原点、`devicePixelRatio` 和窗口 DPI，按客户区宽高比居中 `contain`，不得缩在孔径左上角或露出背后的空状态。中央画面内只保留动态识别框、必要告警和右上角单行 `采集 FPS · 识别 FPS · 端到端延迟`；后端、provider、内存、队列、丢帧和分项耗时放入侧栏或日志，不得以多行面板遮挡游戏内容。
...
为避免首次地图验证形成循环依赖，`MapCalibrating` 是地图标定流程的唯一例外：生产输入合同及实机验收已经通过时，它可以在当前可见且几何安全的平台范围内执行单次不超过 300ms 的校准移动，只允许 `MoveLeft`、`MoveRight`、`Jump` 和立即 `ReleaseAll`，禁止攻击、补给、拾取和跨未知边界。每次动作后必须重新观察，任何不确定立即暂停。该动作只生成校准证据，不代表 candidate 地图可用于生产导航；没有已验收生产输入时由用户手动移动角色完成扫描。8.3.1 的用户监督战斗试运行是独立的 Windows 验收入口，不属于地图标定，也不能升级 candidate 地图。
...
为建立 Windows 实机闭环证据，允许用户在停止或暂停态显式启动最长 15 秒的 `CombatTrial` 受监督会话上下文。它不新增公共 `SessionState`，底层继续使用 `Navigating`、`Attacking` 和 `Paused`；它不是生产自动运行，也不能自动恢复。试运行只允许当前可见平台内的 `MoveLeft`、`MoveRight` 和 `Attack(profileId=singleAttack)`；单次移动保持 80-250ms，攻击保持由 60-120ms 档案边界和最新反馈约束，当前默认 80ms，禁止跳跃、攀爬、补给、拾取和跨未知边界。

该模式允许在完整地图尚未 validated 时工作，但必须先通过前台、窗口身份、帧 TTL、Broker/IPC/心跳、角色脚底附近连续支撑表面得出的左右本地安全边界、连续三帧稳定 Monster，以及唯一 Self 的受控水平位移确认。无法得到本地安全边界时不得启动。单帧原始置信度可以低于生产动作阈值，但融合后的唯一身份结果仍必须达到动作门槛；这不是忽略安全门。目标只取 Self 脚底高度带内的最近 Monster。每个短动作后必须等待新画面，进入攻击距离、接近边界、目标/Self 丢失或反馈异常时立即释放；连续两次攻击无反馈、15 秒到期或任一安全条件失败时 `ReleaseAll` 并暂停。F9/F12 保持最高优先级。
...
共享 `BrokerProtocol`、Host `BrokerClient`/adapter/executor、`Maple.InputBroker.exe` 入口及 broker 自主 `ReleaseAll`/watchdog 当前为 `SOURCE_READY`：源码、替身测试和 Windows Release 构建已通过，Host 已组合 `BrokerInputAdapter`，但默认保持 `BROKER_NOT_ARMED`；这不代表前台门禁、原生热键、发布包、授权客户端动作或 L4/L5 已完成。portable contract 从本版本起直接检查这些源码边界。
...
此前的 VHF 驱动实验不再属于当前产品或发布路径，其源码、安装工具、夹具和验收命令均已移除。历史构建记录只保留在验证文档中，不能恢复为生产依赖，也不能替代 broker 的安全验收。
...
7. **发布验证**：分别打包普通权限 Host 和显式提权 broker，完成 IPC 身份、日志脱敏、崩溃恢复、30 分钟/4 小时/8 小时 soak 和回滚。
...
生产输入另需发布 `Maple.InputBroker.exe` 并记录普通/管理员完整性级别、IPC、前台、扩展扫描码、F9/F12、所有 ReleaseAll 触发器和授权客户端画面反馈。发布前先运行 `tests/windows/production_input_contract.tests.ps1`，真实动作和异常释放证据按当前 Windows 实施计划采集。
