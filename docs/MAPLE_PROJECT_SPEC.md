...
后续所有开发、修复、验证和交接必须直接在 `master` 分支进行。不得创建或切换到功能分支、临时开发分支或独立工作树继续实现；如当前工作区不在 `master`，必须先回到 `master` 再修改代码、文档或测试。除非本文后续版本明确改写本规则，否则任何计划、交接或工具默认分支策略都不能覆盖该要求。
...
- 左侧：HP/MP 当前读数及阈值、攻击模式、攻击技能键、移动/跳跃键、拾取开关/按键、地图档案和只读的随机节奏倒计时；
...
- 定点压键攻击每轮只允许一次攻击 key-down 和一次对应 key-up，按 8.5 节的加权随机时长持续压住攻击键；不得用轮内重复点击模拟该模式；
...
5. 普通短按攻击的按下/保持时间由技能配置的上下限、冷却、动画/命中反馈和目标状态计算；8.5 节的定点压键模式改用独立的加权长按范围，但仍受同一观察和安全门约束。攻击后重新观察目标数量、目标位置、角色位移和技能反馈，再决定继续攻击、重新移动、切换目标或暂停。
...
定点攻击使用有边界、可回放的随机阶段状态机：`AttackHolding -> RepositionFirst -> RepositionGap -> RepositionSecond -> OptionalRest -> AttackHolding`。随机值只由后端产生，React 只能显示状态和倒计时，不能生成动作或提交按键。

每轮攻击只发送一次攻击 key-down，持续压住至计划截止时间后发送一次 key-up。计划保持时间按整数毫秒独立抽样：88% 落在 `[20_000, 30_000]ms`，9% 落在 `[10_000, 20_000)ms`，3% 落在 `[1_000, 10_000)ms`。本模式不依赖纯图色统计攻击次数、命中次数或动画次数。普通短按技能仍保留自己的短按上下限；只有定点压键 `Attack` 可以使用最长 30,000ms 的动作限制，其他动作不得继承该限制。

攻击截止或安全提前释放后执行两段相反方向的短移动。安全允许时首方向按 50%/50% 抽取左或右，第二段使用相反方向；两段保持时间分别独立抽取 `[60, 400]ms`，段间无按键间隔抽取 `[50, 350]ms`。平台边界、最新视觉反馈和安全门可以缩短、拒绝或改变随机移动，安全约束优先于随机分布。

移动完成后独立进行休息判定：25% 概率无按键休息 `[2_000, 5_000]ms`，75% 概率直接开始下一轮。休息不是 `Pause` 动作；休息期间保持全键释放并继续检查停止、失焦、帧新鲜度、设备健康和 EmergencyStop。

攻击长按期间必须持续消费最新观察并重复执行安全门，不能用固定反馈帧数替代单调截止时间。目标消失、窗口失焦、帧过期、安全门失败、停止、取消、异常或设备断连都必须立即 key-up，并在退出路径执行 `ReleaseAll`。所有权重分段、计划/实际时长、方向、移动时长、间隔、休息判定和提前释放原因都写入 JSONL。

Host 必须发布版本化 `combat.rhythm.updated` 事件，至少包含周期编号、当前阶段、抽中的阶段总时长、剩余毫秒、单调更新时间和可选的提前释放原因。阶段切换时立即发布，活动阶段约每 250ms 更新一次。工作台显示当前阶段、本阶段抽中的时长和实时倒计时；停止、暂停或 EmergencyStop 后不得继续显示旧倒计时。
...
```json
{
  ...
  "combat": {
    ...
    "stationaryRhythm": {
      ...
      "reposition": { "minHoldMs": 60, "maxHoldMs": 400, "minGapMs": 50, "maxGapMs": 350 },
      "rest": { "probability": 0.25, "minMs": 2000, "maxMs": 5000 }
    }
  },
...
`combat.stationaryRhythm` 的攻击权重总和必须为 100，所有时间使用整数毫秒，所有范围必须满足最小值不大于最大值。首版使用规范中的固定验收值，不要求 UI 开放概率和范围编辑。
