...
import type { HostEvent, ObservationSnapshot, TelemetrySnapshot, UiCommand } from '../../contracts/bridge'
...
export function PreviewRegion({ preview, observation, telemetry, onRequestSnapshot, sendCommand }: { preview: PreviewAvailability; observation?: ObservationSnapshot; telemetry?: TelemetrySnapshot; onRequestSnapshot(): void; sendCommand(command: UiCommand): void }) {
  ...
  return (
    <main className="preview-panel">
      ...
      <div ref={nativePreviewRef} className={`preview-stage ${connected ? 'preview-stage--connected' : ''}`} aria-label="原生预览画面区域">
        ...
        <div className="preview-hud preview-hud--top"><span><i className={`hud-dot ${connected ? 'hud-dot--good' : ''}`} />{sourceLabel}</span><span>{telemetry ? `采集 ${Math.round(telemetry.captureFps)} FPS · 识别 ${Math.round(telemetry.recognitionFps)} FPS · ${Math.round(telemetry.frameLatencyMs)} ms` : connected ? '等待首帧性能数据' : '画面源未连接'}</span></div>
